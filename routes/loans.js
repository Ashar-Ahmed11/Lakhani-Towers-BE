...
router.post('/', fetchAdmin, [
  ...
  try {
    const created = await Loan.create(req.body);
    res.json(created);
  } catch {
...
router.get('/', fetchAdmin, async (req, res) => {
  try {
    ...
    const list = await Loan.find(query).sort({ createdAt: -1 }).populate({ path: 'to', select: 'userName userMobile' });
    res.json(list);
  } catch {
    res.status(500).json({ message: 'Server error' });
  }
...
router.get('/:id', fetchAdmin, async (req, res) => {
  try {
    const loan = await Loan.findById(req.params.id).populate({ path: 'to', select: 'userName userMobile' });
    res.json(loan);
  } catch {
    res.status(500).json({ message: 'Server error' });
  }
